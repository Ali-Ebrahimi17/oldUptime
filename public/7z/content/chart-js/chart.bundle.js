	...
	*/
	 ...
	 */
			updateDataset             : function(index) {
				...
				if (
					core_plugins.notify(me, 'beforeDatasetUpdate', [ args ]) === false
				) {
					return
				}
	 ...
	 */
			drawDatasets              : function(easingValue) {
				...
				if (
					core_plugins.notify(me, 'beforeDatasetsDraw', [ easingValue ]) ===
					false
				) {
					return
				}
 ...
 */
	 ...
	 */
		...
		helpers$1.getRelativePosition = function(evt, chart) {
			...
			mouseX = Math.round(
				(mouseX - boundingRect.left - paddingLeft) /
					width *
					canvas.width /
					chart.currentDevicePixelRatio,
			)
			mouseY = Math.round(
				(mouseY - boundingRect.top - paddingTop) /
					height *
					canvas.height /
					chart.currentDevicePixelRatio,
			)
	 ...
	 */
		...
		getBaseValue                : function() {
			...
			return me.beginAtZero
				? 0
				: min < 0 && max < 0 ? max : min > 0 && max > 0 ? min : 0
		},
	 ...
	 */
		getPixelForOffset    : function(time) {
			...
			var start = me._horizontal
				? isReverse ? me.right : me.left
				: isReverse ? me.bottom : me.top
			var pos = interpolate$1(me._table, 'time', time, 'pos')
			var offset =
				size *
				(me._offsets.start + pos) /
		...
		draw                : function() {
			...
			if (opts.display) {
				...
				if (isHorizontal) {
					cursor = {
						x    :
							me.left + (legendWidth - lineWidths[0]) / 2 + labelOpts.padding,
